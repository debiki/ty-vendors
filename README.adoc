...
git commit -m "Upgrade lua-resty-acme, for OpenResty X.Y."

# Review the changes. Then:
git push origin ...

cd ../../..
git add images/web/openresty-pkgs
git commit -m "Upd OpenResty packages submod, for OpenResty X.Y."
----

Comment out the read-write mount from  in docker-compose.yml again.

Lastly, rebuild the Web image, so the new packages get copied into it:
